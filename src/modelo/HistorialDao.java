...
public class HistorialDao implements Crudhistorial<Historial> {
    ...
    public int setAgregar(Historial h) {
        String sql = "INSERT INTO historial(id_usuario, id_producto, fecha, cantidad_compra, precio_total,id_metodo_pago) VALUES (?,?,?,?,?,?)";

        try (
                Connection con = Conexion.getInstance().getConnection();
                PreparedStatement ps = con.prepareStatement(sql)) {
            ps.setInt(1, h.getId_usuario());
            ps.setInt(2, h.getId_producto());
            ps.setString(3, h.getFecha());
            ps.setInt(4, h.getCantidad_compra());
            ps.setDouble(5, h.getPrecio_total());
